...
let rec choose etc things = 
    if things <> [] then
      etc (hd(things));
    if things <> [] then
      choose etc (tl(things));;

let rec allbut things thing = 
  if things = [] then
    things
  else if hd(things) = thing then
    tl(things)
  else 
    hd(things)::allbut (tl(things)) thing;;

let permute etc things =
  let permutedThings = [] in
  let rec permuting etc permutedThings unpermutedThings =
    if unpermutedThings = [] then
        etc permutedThings
    else
       choose (fun thing -> permuting etc ((thing)::(permutedThings)) (allbut unpermutedThings thing)) unpermutedThings
    in permuting etc permutedThings things;; 
